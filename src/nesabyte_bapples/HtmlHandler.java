...
public class HtmlHandler {
  /**
   * this method takes in the html file, reads the file and store it in a string. then the long
   * string will be validated to take all the links this method reads from the url then store all
   * its html resource contents in a string. The string then is processed by the use of the
   * pullLinks() method to take all of the links and be placed inside a HashSet. Then send the
   * Hashset in FindUnsecured() to find all the links with http and turn them into https and check
   * their status
   *
   * @param file
   */
  public static void checkSecureHTML(String file) {
    ...
    try {
      ...
      // string then is processed by the use of the pullLinks() method to take all of the links and
      // be placed inside a HashSet.
  ...
  /**
   * this method takes in 2 parameters: 1.) a string which is the absolute pathway of the .html
   * file. 2.) an in for the statcode, if the user wants to search for a specific status code.
   *
   * <p>this method reads from the *.html file then store all its contents in a string. The string
   * then is processed by the use of the pullLinks() method to take all of the links and be placed
   * inside a HashSet. Then each of the link will be passed into the AppleCode() method to check all
   * the Status code. then it prints out the link with its status code.
   *
  ...
  /**
   * this method takes in 2 parameters: It get the list of ignore from txt file, and the request url
   * list are from html file. the request url will get rid of the list of ignore urls.
   *
   * @author Eunbee Kim
   * @param txtFile
   * @param htmlFile
  ...
  public static void classifyingTXT(String txtFile, String htmlFile, int statcode) {
    ...
    try {
      ...
      // string then is processed by the use of the pullLinksExcept() method to take all of the
      // links and be placed inside a HashSet.
